...
export default function Calendar({
	...
	const [startPaymentFrontend, setStartPaymentFrontend] =
		useLocalStorageTX2<StartPaymentFrontend>('startPaymentFrontend.v2', {
			adSubPagesData: {},
			formFieldData: {},
	...
	useEffect(() => {
		if (startPaymentFrontend?.adSubPagesData) {
			const secondLevelCategory = adSubPageId
				? startPaymentFrontend?.adSubPagesData[adSubPageId]
				: null
			const planIds = secondLevelCategory
				? Object.keys(secondLevelCategory)
				: null
			const _planId = planIds ? planIds[0] : null
			if (_planId) {
				setPlanId(_planId)
			}
		}
					...
					return Object.entries(adSubPageData.eventTimes ?? {}).map(
						([key3, value3]) => {
							return {
								isBasket: true,
								id: key3,
								title: auth.userDTO?.userName ?? '',
								start: value3.startTime,
								end: value3.endTime,
								editable: true,
								extendedProps: {
									productSlug: adSubPageId,
									eventTimeId: key3,
								},
	...
	/*eventInputs = isCheckoutVisible
		? [...additionalEventTimes]
		: [...additionalEventTimes, ...eventInputs]*/
				...
				<div id="calendar">
					<FullCalendar
						...
						eventClick={(clickInfo) => {
							...
							if (eventTimeEditor) {
								eventTimeEditor.scrollIntoView({ behavior: 'smooth' })
							}
						}}
						editable={true}
						eventTimeFormat={{
